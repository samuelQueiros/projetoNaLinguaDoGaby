...
function Contas(){
 ...
 return <><Title title="Contas a pagar" subtitle="Lançamentos, aprovação e situação financeira." action={<button onClick={()=>setEditing({desconto:0,juros:0,multa:0})}>Nova conta</button>}/>{msg&&<div className="alert">{msg}</div>}<section className="card"><Table rows={(data||[]).map(r=>({...r,_aviso:contaComFornecedorInativoPendente(r)}))} onClick={x=>setEditing({...x})} rowClassName={r=>contaComFornecedorInativoPendente(r)&&'row-warning'} cols={[['','_aviso','warn'],['Descrição','descricao'],['Fornecedor','fornecedor.razaoSocial'],['Vencimento','vencimento','date'],['Valor final','valorFinal','money'],['Aprovação','statusAprovacao'],['Financeiro','statusFinanceiro']]}/></section>
  {editing&&<div className="modal"><form className="dialog" onSubmit={salvar}><h2>{editing.id?'Detalhe da conta':'Nova conta'}</h2>{editing.id&&<p><b>Aprovação:</b> {editing.statusAprovacao} · <b>Financeiro:</b> {editing.statusFinanceiro}</p>}<div className="form-grid">
   ...
   </div>{avisoFornecedorInativo&&<p className="alert warning">⚠️ Fornecedor inativo — aprovar ou rejeitar esta conta continua permitido normalmente.</p>}<div className="actions">{editing.id&&<><button type="button" className="success" onClick={()=>action('aprovar')}>Aprovar</button><button type="button" className="danger" onClick={()=>action('rejeitar')}>Rejeitar</button></>}<button type="button" className="secondary" onClick={()=>setEditing(null)}>Cancelar</button><button>Salvar</button></div></form></div>}
 </>}
