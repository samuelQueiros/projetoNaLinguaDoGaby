...
| Camada | Tecnologia |
|---|---|
| Backend | ASP.NET Core 8 (API HTTP + Identity) |
...
│   (API + React SPA, porta 8080)│──HTTP─▶│   (FastAPI, porta 8000)   │
│                               │  X-Internal-Token              │
...
│   └── ErpFinanceiro.Web/             # API ASP.NET — serve o build do frontend em wwwroot/react
├── tests/
│   └── ErpFinanceiro.Tests/           # testes xUnit (174 testes)
...
[`backend/servico-documentos/README.md`](backend/servico-documentos/README.md)
para detalhes do pipeline, endpoints e pontos de extensão (novo tipo de
documento, novo formato de arquivo, etc.).
...
Pré-requisitos: .NET 8 SDK, Node.js 20+ com npm e Docker (para o PostgreSQL e, opcionalmente, o serviço de documentos).
...
cp backend/src/ErpFinanceiro.Web/appsettings.Development.json.example \
   backend/src/ErpFinanceiro.Web/appsettings.Development.json

# 3. Aplique as migrations
dotnet tool install --global dotnet-ef
dotnet ef database update \
  -p backend/src/ErpFinanceiro.Infrastructure/ErpFinanceiro.Infrastructure.csproj \
  -s backend/src/ErpFinanceiro.Web/ErpFinanceiro.Web.csproj

# 4. Compile o frontend React
cd frontend
npm ci
npm run build
cd ..

# 5. Rode a API e a SPA
dotnet run --project backend/src/ErpFinanceiro.Web
```

Para desenvolvimento com hot reload, rode npm run dev em frontend/ em um
terminal e o backend .NET em outro. O Vite encaminha chamadas /api ao
ASP.NET. O dotnet publish e os Dockerfiles executam o build React
automaticamente.
...
[`backend/servico-documentos/README.md`](backend/servico-documentos/README.md)
para rodá-lo fora do Docker.

## Testes

```bash
# Backend .NET (174 testes)
dotnet test backend/tests/ErpFinanceiro.Tests/ErpFinanceiro.Tests.csproj

# Serviço de documentos (Python)
cd backend/servico-documentos && pytest
```
...
```bash
# Na máquina gerenciada pelo Portainer, na raiz do repositório:
docker build -t erp-financeiro-web:1.0.0 .
docker build -t erp-financeiro-documentos:1.0.0 backend/servico-documentos
...
descreve os testes já realizados sobre essa configuração (build, 174 testes
...
- Mantenha **uma única instância** da web: o processamento de documentos usa uma fila em memória e ainda não está preparado para múltiplas réplicas.
...
├── frontend/                        # SPA React/TypeScript (Vite)
├── backend/
│   ├── src/                         # solution .NET (ver Arquitetura)
